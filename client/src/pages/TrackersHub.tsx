/*
 * Redesign v2 — "Keepsake Field Guide" logs hub (tab label: Logs).
 * Grouped into Daily care / Health & vet / Growing up,
...
import { ASSETS, isPreHomecoming } from "@/content/wobbles";
              ...
              <div className="keepsake-card overflow-hidden divide-y divide-border/50">
                {members.map((t) => (
                  <div key={t.id} className="flex items-center gap-2 pr-3">
                    <Link
                      href={`/trackers/${t.id}`}
                      className="flex items-center gap-3.5 pl-4 py-3.5 press-scale bg-transparent min-w-0 flex-1 min-h-[44px]"
                    >
